...
router.post('/', auth, async (req, res) => {
  if (req.user.role !== 'client') return res.status(403).json({ error: 'Apenas clientes podem reservar' });

  const { serviceId, date, customerName, customerEmail, notes } = req.body;
...
router.get('/', auth, async (req, res) => {
  ...
  const reservations = await Reservation.find({ business: { $in: businessIds } })
    .populate('service', 'name duration price')
...
router.put('/:id/status', auth, async (req, res) => {
  ...
  const reservation = await Reservation.findById(id).populate('business');
...
router.put('/:id', auth, async (req, res) => {
  ...
  if (reservation.status !== 'pending')
    return res.status(400).json({ error: 'Só reservas pendentes podem ser modificadas' });
